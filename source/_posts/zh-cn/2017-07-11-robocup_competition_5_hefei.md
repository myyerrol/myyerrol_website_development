...
- #### RoboCup Logistics
  ...
  **改革**
  RoboCup物流组从2015合肥国际赛开始进行改革，引入了全新的挑战——机器人不再像前几届那样需要在地板上移动冰球，而是根据[Festo](http://www.festo.com/)提供的模块化生产系统（MPS）来操作工作站。如下图所示，MPS是用于处理气瓶的小型生产机器，它由彩色的底座，零个或最多三个彩色的圆环组成。

  这些气瓶在工厂场景中体现了产品。 通过对这些产品的物理操纵，货物的物料流动和生产顺序变得可以理解。 订购的产品随机确定，并在游戏中动态发布。 因此，由机器人队伍驱动的生产过程的规划和调度是这种情况的关键方面，以及机器人技术中的典型问题，如具有碰撞避免，自定位，对象检测或（有限）操纵的导航。
