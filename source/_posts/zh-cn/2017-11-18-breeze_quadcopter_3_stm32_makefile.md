...
- #### 编译命令
  ...
  **伪目标定义**
  ...
  首先我先来解释一下什么是**伪目标**。伪目标其实并不是一个**文件**，而是一个**标签**，所以GNU/Make无法生成它的依赖关系并决定它是否要执行，只有通过我们显示地指明这个目标才能让其生效。当然，伪目标的取名不能和文件名重名，不然其就失去伪目标的意义了。因此，为了避免其和文件重名的情况发生，我们可以使用一个特殊的标记**.PHONY**来显示地指明一个目标是伪目标，向GNU/Make说明，不管是否有这个文件，这个目标就是伪目标。
